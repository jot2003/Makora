    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-sm animate-fade-in-up">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">Get started with Meeting Copilot</p>
        </div>
        ...
        <div className="space-y-2 mb-6">
          <button
            ...
            className={cn(
              ...
              !oauthProviders.google ? 'opacity-40 cursor-not-allowed' : 'hover:bg-accent cursor-pointer'
            )}
          >
            ...
            {!oauthProviders.google && <span className="text-[9px] text-muted-foreground/60 ml-1">(not configured)</span>}
          </button>
          <button
            ...
            className={cn(
              'w-full h-10 rounded-xl border border-border bg-card text-sm font-medium flex items-center justify-center gap-2 transition-colors',
              !oauthProviders.github ? 'opacity-40 cursor-not-allowed' : 'hover:bg-accent cursor-pointer'
            )}
          >
            {oauthLoading === 'github' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Github className="w-4 h-4" />}
            Continue with GitHub
            {!oauthProviders.github && <span className="text-[9px] text-muted-foreground/60 ml-1">(not configured)</span>}
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Username" value={username}
              onChange={(e) => { setUsername(e.target.value); clearError() }} required
          ...
          <div className="relative">
            ...
            <input type={showPw ? 'text' : 'password'} placeholder="Password (min 6 chars)" value={password}
              onChange={(e) => { setPassword(e.target.value); clearError() }} required minLength={6}
